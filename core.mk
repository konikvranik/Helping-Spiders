...
FLAGSSUFFIX=-DF_CPU=80000000L -DLWIP_OPEN_SRC -DARDUINO=10608 -DARDUINO_$(BOARD) -DARDUINO_ARCH_$(PLATFORM) -DARDUINO_BOARD=\"$(BOARD)\" -D$(PLATFORM) -DWIFI_SSID=\"$(WIFI_SSID)\" -DWIFI_PASSWORD=\"$(WIFI_PASSWORD)\" -DNODE_ID=\"$(NODE_ID)\" $(addprefix -DENABLE_,$(MODULES)) $(addprefix -DINIT_STUFF=,"$(INIT_STUFF)") $(FLAGS) -include $(CORE_PATH)/src/config.h
...
upload: $(MODULE)
	curl -X POST --progress-bar -F "update=@$(MODULE)" http://$(MODULE_IP)/update >&2
	rm $(MODULE)
.PHONY: upload
