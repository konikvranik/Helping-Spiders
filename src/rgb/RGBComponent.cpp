...
RGBComponent::RGBComponent(const uint8_t sensor_id, const uint16_t red_pin,
                           const uint16_t green_pin, const uint16_t blue_pin)
    : rgbBlender(red_pin, green_pin, blue_pin), AbstractComponent(sensor_id) {
  light_status_msg = MyMessage(sensor_id, V_STATUS);
  dimmer_msg = MyMessage(sensor_id, V_PERCENTAGE);
  rgb_msg = MyMessage(sensor_id, V_RGB);
  candle_msg = MyMessage(sensor_id, V_STATUS);
  mode_msg = MyMessage(sensor_id, V_TEXT);
}
...
void RGBComponent::presentation() {
  present(sensor_id, S_RGB_LIGHT, "LED");
  present(sensor_id, S_INFO, "LED mode");
}
  ...
  if (result) {
    ...
    case MODE_CANDLE:
      rgbBlender.Blend(rgbBlender.GetColor(),
                       candle.color_list[candle.RandColor()],
                       candle.RandTimer());
      result = false;
      break;
    case MODE_DAYTIME:
  ...
  if (!result) {
    ...
    if (result) {
      ...
      case MODE_CANDLE:
        ...
        break;
      case MODE_DEFAULT:
        Log.notice("Finished color %s%s" CR,
                   c2s(rgbBlender.GetColor()).c_str());
        if (light_state == LIGHT_ON)
          rgbBlender.Hold(rgb);
        else
          rgbBlender.TurnOff();
...
void RGBComponent::receive(const MyMessage &message) {
...
const String RGBComponent::c2s(const Color color) {
  const char *fmt = "%06x";
  long clr = color.red * 0x10000 * 0x100 / RGB_MAX_VALUE +
             color.green * 0x100 * 0x100 / RGB_MAX_VALUE +
             color.blue * 0x100 / RGB_MAX_VALUE;
  char c1[1];
...
void RGBComponent::blend(Color c) {
  ...
  if (light_state == LIGHT_ON) {
    Log.notice("Light state: ON, Level: %d, color: %s" CR, c2b(c),
               c2s(c).c_str());
  } else {
    mode = MODE_DEFAULT;
    Log.notice("Light state: OFF" CR);
    c = _BLACK;
  }
  Log.notice("Blending from %s to %s" CR, c2s(rgbBlender.GetColor()).c_str(),
             c2s(c).c_str());
  rgbBlender.Blend(rgbBlender.GetColor(), c, (uint32_t)BLEND_TIME);
  result = false;
}

void RGBComponent::setup() { rgbBlender.Hold(_BLACK); }
...
void RGBComponent::reportStatus(JsonObject &jo) {
  ...
  c["colorString"] =
      this->c2s(this->candle.color_list[this->candle.rand_color]);
  c["delay"] = this->candle.timer;
  jo["Color"] = "0x" + this->c2s(this->rgb);
  jo["Real color"] = "0x" + this->c2s(this->getColor());
}

const Color RGBComponent::getColor() { return rgbBlender.GetColor(); }

String RGBComponent::moduleName() { return "RGB"; }
